...
def upload_short(
    ...
    Raises RuntimeError on token expiry, quota exhaustion, or privacy mismatch.
    """
    ...
    try:
        ...
        if "quotaExceeded" in error_str or (hasattr(e, "status_code") and getattr(e, "status_code", None) == 403):
            raise RuntimeError(
                f"YouTube API quota exceeded. Wait until quota resets (midnight Pacific) or request a quota increase "
                f"in Google Cloud Console. Original error: {e}"
            )
        raise
    ...
    if privacy == "public" and returned_privacy != "public":
        raise RuntimeError(
